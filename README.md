...
A snapshot of the current question bank (25 verified questions) lives in
`supabase/seed_questions.sql`. It is safe to run on a fresh database — it skips
itself if `questions` already has rows.

## 4. Store assets & listing

- **App icon / splash / favicon** live in `mobile/assets/` and are wired in
  `mobile/app.json`. They are generated by `mobile/scripts/generate-assets.py`
  (`pip install Pillow && python3 scripts/generate-assets.py`) — tweak the brand
  colors/shape there and regenerate. Replace with professional artwork before a
  public launch.
- **Privacy policy:** `docs/PRIVACY_POLICY.md` — publish it at a public URL
  (e.g. GitHub Pages) and paste the link into App Store Connect (required).
- **Store listing copy** (name, subtitle, description, keywords, privacy
  labels): `docs/APP_STORE_LISTING.md` — copy field-by-field into App Store
  Connect.
